...
resetBtn.addEventListener("click", function () {
	hasDrawn = false;
	const resetQuestion = confirm("是否要重製問題？")
	if (resetQuestion) {
		questionInput.value = "";
	}
	...
	allCards.forEach(function (card) {
		...
		setTimeout(function () {
			const cardFront = card.querySelector(".card-front");
			cardFront.innerHTML = "[placeholder]";
...
function preloadTarotImages() {
	tarotDeck.forEach(function (card) {
		const img = new Image();
		img.src = card.image;//自動下載並快取這張圖片
	});
}

//當網頁載入完成後，進行預載
window.addEventListener("DOMContentLoaded", preloadTarotImages);